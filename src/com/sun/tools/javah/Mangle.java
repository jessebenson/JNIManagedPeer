...
public class Mangle {
	...
	public final String mangle(CharSequence name, int mtype) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (isalnum(ch)) {
				result.append(ch);
			} else if ((ch == '.') && mtype == Mangle.Type.CLASS) {
				result.append('_');
			} else if (( ch == '$') && mtype == Mangle.Type.CLASS) {
				result.append('_');
				result.append('_');
	...
	public String mangleMethod(ExecutableElement method, TypeElement clazz, int mtype) throws TypeSignature.SignatureException {
		...
		if (mtype == Mangle.Type.METHOD_JDK_1) {
			...
			result.append('_');
			result.append(mangle(method.getSimpleName(), Mangle.Type.FIELD));
			result.append("_stub");
			return result.toString();
		}
		...
		result.append('_');
		result.append(mangle(method.getSimpleName(), Mangle.Type.JNI));
